...
object movie_recom {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def recom_movie(iterator: Iterator[(String, Array[(Int, Int)])]): Iterator[(String, Array[String], Array[(String, Double)])] = {
      var result = new ArrayBuffer[(String, Array[String], Array[(String, Double)])]() //new ArrayBuffer[(String, Array[(String, Int)], Array[(String, Double)])]()
      ...
      for ((user, movie_prefer_array) <- iterator) {
        ...
        val watched = all_watched_movie.keySet.map(r => {
          if (idmap_br.value.keySet.contains(r)) {
            idmap_br.value(r)
          } else {
            ""
          }
        }).toArray
        result += ((user, watched, recom_movie_map.toArray.sortWith(_._2 > _._2).take(num)))
      }
      result.iterator
    }
    ...
    for ((u, watched, recom) <- user_recommovie.take(100)) {
      print(u + "观看的视频:")
      for (w <- watched) {
        print(w + ", ")
      }
    ...
    //    println(sdf_time.format(new Date((System.currentTimeMillis()))) + "***********************get recom movie" + user_watchedmovie_recommovie.count() + "*****************************")
  ...
  def item_similarity_rdd(user_movie_prefer: RDD[(String, Array[(Int, Int)])], max_index: Int, simp: Double, sim_movie_num: Int, normalization: Boolean): RDD[(Int, Array[(Int, Double)])] = {
    ...
    println(sdf_time.format(new Date((System.currentTimeMillis()))) + "*********************** n_length=" + n.length + " ,n_max= " + n.max + " ,n=0的个数：" + cnt + "*****************************")
  ...
  def save_data(table_out: String, user_recommovie: RDD[(String, Array[String], Array[(String, Double)])]): Unit = {
    ...
    val structType = StructType(
      StructField("user", StringType, false) ::
        StructField("watched", StringType, false) ::
    ...
    val create_table_sql: String = "create table if not exists " + table_out + " ( user String,watched String,sim_items String) partitioned by (stat_date bigint) stored as textfile"
  ...
  def load_movie(): (RDD[(String, Array[(Int, Int)])], Map[Long, String], Int) = {
    ...
    val sql_2 = "select column_index,name from  algo.personal_recommend_usecollect_items_16 where column_index is not null and stat_date=" + date1
